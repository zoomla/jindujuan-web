...
if file.fileSize<10 or file.fileSize>50000*80000 then
...
ufp="link_"&year(now)&month(now)&day(now)&hour(now)&minute(now)&second(now)&"."&upfilename(ubound(upfilename))
file.saveas Server.mappath("../../img_info/friend_bill/"&ufp)
%>
<script >
parent.form1.big_image.value+='<%=ufp%>'
location.replace('link_upload_01.asp')
</script>
<%
set file=nothing
set upload=nothing
