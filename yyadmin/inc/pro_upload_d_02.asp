...
if file.fileSize<10 or file.fileSize>500*800 then
...
ufp="pro_"&year(now)&month(now)&day(now)&hour(now)&minute(now)&second(now)&"."&upfilename(ubound(upfilename))
file.saveas Server.mappath("../../pro_img/"&ufp)
%>
<script >
parent.form1.small_image.value+='<%=ufp%>'
location.replace('pro_upload_d_01.asp')
</script>
<%
set file=nothing
set upload=nothing
